...
```{r wildfire} LOOK AT EXERCISES FOR HELP IF NEEDED
# filter to tweets concerning wildfires
tweets <- tibble::tibble(text = senators_data$text,
                           date = senators_data$created_at,
                           user = senators_data$user)
wildfire <- stringr::str_detect(string = hashtags,
                                pattern = "wildfire")
wildfire_tweets <- tweets %>%
  dplyr::filter(wildfire = TRUE)
```
