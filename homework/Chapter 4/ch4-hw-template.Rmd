...
ozone_data_all <- readr::read_csv(file = csv_path) 
ozone_data_drop <- sum(is.na(ozone_data_all$sample_measurement))
```
...
`ggplot()`? 
``` {r plot in R}
plot(x = ozone_data$datetime, 
     y = ozone_data$ozone_ppm,
     xlab = "datetime",
     ylab = "ozone_ppm")
```
...
ozone_ggplot <- ggplot(ozone_data, 
                          aes(x = datetime,
                              y = ozone_ppm))
```
...
ozone_ggplot <- ozone_ggplot + geom_point()
```
...
--> ggplot has 1 layer: geom point layer
...
ozone_ggplot <- ozone_ggplot + theme_classic()
```
...
ozone_ggplot <- ozone_ggplot + labs(x = "Date Time",
                                    y = "Ozone Concentration (ppm)",
                                    title = "Ozone Measurements in Fort Collins by Hour")
```
...
ozone_ggplot <- ozone_ggplot + aes(alpha = 0.2)
ozone_ggplot
```
...
high_ozone <- filter(ozone_data, ozone_ppm > 0.070) 

# time series of high ozone measurements
high_ozone_plot <- ggplot(high_ozone, 
                          aes(x = datetime,
                              y = ozone_ppm)) +
                            geom_point()
high_ozone_plot
...
--> a higher level of ozone occurs near the begining of July. This makes sense as this is peak summer time and most people are on vacation and traveling. 
...
ozone_perc <- (nrow(high_ozone) / nrow(ozone_data))*100
print(paste(round(ozone_perc, digits = 3), "% exceeding 0.070 ppm")) # rounded percentage of measurements that exceed 0.07 ppm
```
...
and why the extension would improve the data visualization and communication.
  --> ggaminate is a theme that could be beneficial for my senior design project. I am on a team that is developing a fully automated hydroponic greenhouse. This greenhouse will be monitoring levels of nutrient through EC sensors as well as other sensors for pH, water pressure, and sunlight. All of these could benefit from ggaminate as it would be able to show the user (there will be a user screen to help the user see how the crops are doing and the levels of water, nutrient, light, and other specs) a visual for the specs predetermined over time. 
\newpage
