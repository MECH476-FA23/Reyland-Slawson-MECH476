...
```{r load-packages and setup}

library(tidyverse)
library(knitr)
knitr::opts_knit$set(root.dir = '/Users/genevieve/Reyland-MECH476/data')
```
...
ozone_data_all <- readr::read_csv(file = "ftc_o3.csv") 
...
ozonedata_max <- dplyr::slice_max(ozone_data, n = 10, order_by = ozone_ppm)
```
...
ozonedata_min <- dplyr::slice_min(ozone_data, n = 10, order_by = ozone_ppm)
```

Do the highest and lowest values tend to occur at certain times of the day?

--> Ozone values are at max around 8/9 pm and the least in the morning around 7am.
...
--> Max value occurs at 20h00 2019-07-03  
--> Min value occurs at 07h00 2019-01-05

```{r high-low}
# extract date/time of highest ozone concentration
ozonedata_maxtime <- ozonedata_max$datetime[1]

# extract date/time of lowest ozone concentration
ozonedata_mintime <- ozonedata_min$datetime[1]
```
...
- estimated atmospheric pressure in Fort Collins, CO (637 mmHg) --> 0.637 Hg
- universal gas constant (22.4136 L) --> 0.0224136 m3
...
```{r convert-ozone}
# create new variable of ug/m3 from ppm and overwrite data set
ozone_weight <- 47.998
c_to_k <- 273.15
atp <- 637
R <- 22.4136
temp <- 10
ozone_numerator <- ozone_data$ozone_ppm*ozone_weight*c_to_k*atp
ozone_denom <- R*(c_to_k+temp)*atp
ozone_ugm3 <- mutate(.data = ozone_data,
                     ozone_ugm3 = 1000*(ozone_numerator/ozone_denom))
